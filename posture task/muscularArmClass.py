...
class muscular_arm():
    def __init__(self, dh=0.01):
        ...
        self.cur_j_state = torch.zeros(9, 4).to(device)
        self.FV = torch.zeros(9,6).to(device)

    def forward(self, x, u, cur_perturbation):
        """
        ...
        self.tor = self.tor.transpose(0,1) + cur_perturbation
        # run the arm dynamics 
        net_command = self.tor
        out, x = self.armdyn(x, net_command)

        # compute cartesian-states from joint-states (Run armkinematics)
        #y = x
        #y = self.armkin(x)
